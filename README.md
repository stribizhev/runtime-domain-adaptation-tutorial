...
## Introduction
Although on-the-fly domain adaptation can be used in various ways, in this tutorial, we consider a scenario that mimics the workflow of a professional translator. We will translate sentences one by one, and after receiving each machine translation, we will imitate post-editing and giving feedback by returning reference translations to the translation system. In turn, the translation system will store feedback in the translation memory. This way, upon receiving the subsequent translation requests, the machine translation model can be adapted using relevant sentences from the translation memory. 
